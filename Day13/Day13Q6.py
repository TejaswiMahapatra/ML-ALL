# 6.	Share your insights on the data based on the clusters
import numpy as np
import pandas as pd
...
plt.show()
cluster_df = pd.DataFrame({'Cluster': cluster_labels, 'Species': df['Species']})
cross_tab = pd.crosstab(cluster_df['Cluster'], cluster_df['Species'])
proportion = cross_tab.div(cross_tab.sum(axis=1), axis=0) * 100
print(proportion)
